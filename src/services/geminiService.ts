...
Instructions:
1. Identify ONLY plastic objects (bottles, containers, bags, wrappers, etc.)
2. Classify each into: PET, HDPE, PVC, LDPE, PP, PS, or Others
3. Return a JSON object with detections.
...
Return ONLY valid JSON. Structure:
{
  ...
  "non_plastic_detected": boolean
      ...
      // With response_mime_type: "application/json", we don't need regex cleaning
      ...
      try {
        ...
        if (parsedResult.detections) {
          detections = parsedResult.detections.map(
            (item: any) => ({
              label: item.label || "Unknown",
        ...
        console.error("Failed to parse JSON response:", e);
        console.log("Raw response:", jsonText);
        toast.error("Failed to process AI results");
        return { annotatedImageUrl: null, detections: [], non_plastic_detected: false };
      }
